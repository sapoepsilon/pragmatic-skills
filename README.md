...
| --- | --- | --- | --- |
| [`pragmatic`](./plugins/pragmatic) | Bundle of all pragmatic-skills, including mobile/backend review workflows and privacy-safe macOS QA capture. | Claude Code | `bundle` `skills` |
...
| [`macos-screen-recorder`](./plugins/macos-screen-recorder) | Privacy-safe macOS QA evidence capture through a user-configured Aqua-session LaunchAgent: signed install verification, clean-desktop staging, screenshot and sampled-frame review, user-owned storage upload, PR evidence, and exact desktop restoration. | Claude Code, Codex | `macos` `qa` `screen-recording` `privacy` `launchagent` `codesign` `evidence` |
